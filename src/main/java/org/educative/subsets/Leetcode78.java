...
public class Leetcode78 {
    ...
    //The inner loop runs setsList.size() times in each iteration of the outer loop. Initially, setsList contains only one empty list, so the inner loop runs 1 time in the first iteration of the outer loop. In the subsequent iterations, the size of setsList increases by a factor of 2 each time, resulting in a total of 1 + 2 + 4 + … + 2^(n-1) iterations of the inner loop. This sum is equal to 2^n - 1, so the time complexity of the inner loop is O(2^n).
